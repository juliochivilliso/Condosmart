    ...
    for (const tx of vencidasOPendientes) {
      ...
      if (tx.estado === 'pendiente' || Number(tx.interes_mora) !== nuevaMora) {
        ...
        if (unidadData && unidadData.usuario_id) {
          ...
          if (notifError) {
            console.error(`❌ Error insertando notificación:`, notifError.message);
          } else {
        ...
        if (userData && userData.email) {
          ...
          console.log(`      Detalle: Estimado(a) ${userData.nombre_completo}, le informamos que su cuota por concepto de '${tx.concepto}' con vencimiento el ${tx.fecha_vencimiento} se encuentra vencida. Se ha acumulado una mora de RD$${nuevaMora.toLocaleString('es-DO')}.`);
        }
      }
    }
